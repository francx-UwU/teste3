...
Ketik ${prefix}verify untuk memulai menggunakan bot.`
  ...
  ferr: 'Lo siento, pero tengo erro con este comando, por favor evite enviar este comando con demasiada frecuencia. 😔🔫',
...
case 'menu':
hasil = `        ────────────────
Hey *${pushname}* intenta escribir ${prefix}menu
        ────────────────`
reply(hasil)
        break
}
...
┃
┃‣ Prefix:  *「 ${prefix} 」*
┃‣ Nombre bot: *francx-bot*
┃‣ WAConnection: *Baileys*
┃‣ Status online: *${kyun(uptime)}*
┃‣ Mengetik: *ON*
┃‣ Group:  *${groupName}*
┃‣ Usuarios : *${_registered.length} Users*
┃‣ Chats : *${totalchat.length} Chats*
┃‣ Github : https://github.com/francx-UwU/
┃‣ Bottom Team Ofc: • _${Wa1}_ •
┃‣ Grupo ?: • _${Wa2}_ •
┃
┃‣ Follow IG: • _${Ig}_ •
╰▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔
...
┃ _Info donasi_
┃‣ ┊*${prefix}Ping*
┃ _Conheça a velocidade de resposta
╰▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔
╭▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂╮
┃  ❥─┯─●*francx-bot*─╝
┃⛨╭╯ 𝚅 2.0
				  ...
				  case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('francx', 'bot ☕🍪')} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								exec(`webpmux -set exif ${addMetadata('francx', 'bot ☕🍪')} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
...
//--Hilih maker
				...
				if (!isRegister) return reply(mess.only.daftarB)
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						exec(`webpmux -set exif ${addMetadata('francx', 'bot ☕🍪')} ${rano} -o ${rano}`, async (error) => {
							if (error) return reply(mess.error.stick)
